...
- Chat list now loads without parsing entire session files, reducing startup time

### Fixed
- Chat files with damaged final lines now remain in chat list and recover on open
...
### Added
- Edit context menu in chat input with Cut, Copy, Paste, and Select All options
- Linux middle-click paste support for primary selection in chat input
- Output tokens/s metric displayed next to context indicator during generation

### Fixed
- Security vulnerabilities in h2 and rustls (RUSTSEC-2026-0258, RUSTSEC-2026-0285)
...
### Fixed
- Stabilize transcript text selection and drag-select behavior
- Prevent message culling during pointer hover and drag-select to ensure selection latches properly
- Retain culling during scrolling to preserve performance
...
### Added
- Agent now allows explicitly requested shell mutations

### Fixed
- Release workflow now keeps dev and master histories aligned
- Release process preserves LF normalization across merges
- Shell scripts enforce LF endings for consistency


## [0.15.1] - 2026-07-16

### Fixed
- Repair macOS packaging
- Enforce LF endings for shell scripts to preserve normalization across merge
...
### Fixed
- Windows: prevent console windows from flashing during headless git and shell tool operations
- Windows: split secrets blob across credential entries to work around CRED_MAX_CREDENTIAL_BLOB_SIZE limit, fixing ChatGPT sign-in persistence
