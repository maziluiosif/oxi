...
/// Record that the control identified by `id` just copied something, so
/// [`copied_recently`] flips it into the transient "Copied" state.
...
/// Frameless "Copy" chip that swaps to a green "Copied" check for a moment after the
/// click — used in code block headers and other tight inline spots.
...
/// "Copy message" context menu with inline feedback: the menu stays open on click so
/// the entry can flip to a green "Copied" check before the user moves on.
...
/// Shared confirmation modal for destructive actions (delete chat/workspace/model,
/// git discard, …): dimmed backdrop, title + body, Cancel + confirm buttons.
/// Enter confirms, Escape / backdrop click / Cancel dismisses.
