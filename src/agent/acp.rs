...
fn fs_read_text(params: &Value) -> Result<String, String> {
    let path = params
        .get("path")
        .and_then(|p| p.as_str())
...
fn fs_write_text(params: &Value) -> Result<(), String> {
    let path = params
        .get("path")
        .and_then(|p| p.as_str())
...
fn emit_update(update: &Value, tx: &StdSender<AgentEvent>) {
...
/// Emit tool output / diff / completion events shared by `tool_call` and `tool_call_update`.
fn emit_tool_content_and_status(update: &Value, tool_call_id: &str, tx: &StdSender<AgentEvent>) {
...
/// Extract the text of an ACP `ContentBlock` (only `text` blocks contribute display text).
...
/// Collapse a `ToolCallContent[]` array into (display text, optional unified diff).
...
/// A minimal unified-diff-style rendering of a replaced text span. Not a real LCS diff — the
/// old span is shown removed and the new span added — but enough for oxi's diff block display.
...
mod tests {
    ...
    fn permission_name_maps_kind() {
        let (name, args) = permission_name_args(
            &json!({"kind":"execute","rawInput":{"command":"ls"},"title":"Run"}),
        );
        assert_eq!(name, "bash");
